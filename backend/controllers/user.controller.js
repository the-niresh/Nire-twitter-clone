...
export const followUnfollowUser = async (req, res) => {
  try {
    ...
    if (isFollowing) {
      ...
      // TODO: send notification to the user. return the id of the user as a response
...
export const updateUser = async (req, res) => {
  const { fullName, email, username, currentPasword, newPasword, bio, link } =
    req.body;
  ...
  try {
    ...
    if ((!currentPasword && newPasword) || (!newPasword && currentPasword)) {
    ...
    if (currentPasword && newPasword) {
      ...
      if (newPasword.length < 6) {
        return res
          .status(400)
          .json({ error: "password is less than 6 charesters" });
      }
    ...
    // pass is null in the response not in the DB
    user.password = null;
